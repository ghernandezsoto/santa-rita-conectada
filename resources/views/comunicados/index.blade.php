                        ...
                        <table class="min-w-full bg-white table-fixed">
                            <thead class="bg-emerald-800 text-white">
                                <tr>
                                    ...
                                    <th class="w-36 py-3 px-4 font-semibold text-sm text-left">Estado</th>
                                    <th class="w-48 py-3 px-4 font-semibold text-sm text-left">Enviado por</th>
                                    <th class="w-48 py-3 px-4 font-semibold text-sm text-left">Acciones</th>
                                </tr>
                            </thead>
                            <tbody class="text-gray-700">
                                @forelse ($comunicados as $comunicado)
                                    <tr class="border-b hover:bg-slate-50 align-middle">
                                        {{-- Título: truncamos en una sola línea --}}
                                        <td class="py-3 px-4 align-middle">
                                            <div class="truncate max-w-md" title="{{ $comunicado->titulo }}">{{ $comunicado->titulo }}</div>
                                        </td>

                                        {{-- Estado: forzamos que no haga wrap --}}
                                        <td class="py-3 px-4 align-middle">
                                            @if ($comunicado->fecha_envio)
                                                <span title="{{ \Carbon\Carbon::parse($comunicado->fecha_envio)->toDateTimeString() }}" class="whitespace-nowrap bg-emerald-200 text-emerald-700 py-1 px-3 rounded-full text-xs">Enviado el {{ \Carbon\Carbon::parse($comunicado->fecha_envio)->format('d/m/Y') }}</span>
                                            @else
                                                <span class="whitespace-nowrap bg-amber-100 text-amber-700 py-1 px-3 rounded-full text-xs">Borrador</span>
                                            @endif
                                        </td>

                                        {{-- Enviado por: truncamos en una sola línea --}}
                                        <td class="py-3 px-4 truncate align-middle" title="{{ $comunicado->user->name }}">{{ $comunicado->user->name }}</td>

                                        {{-- Acciones: quitamos flex-wrap y forzamos no-wrap; los formularios quedan inline-block --}}
                                        <td class="py-3 px-4 align-middle">
                                            <div class="flex items-center gap-2 whitespace-nowrap">
                                                @if ($comunicado->fecha_envio)
                                                    <button type="button" class="copy-btn inline-flex items-center gap-1 text-blue-600 hover:text-blue-800" title="Copiar para WhatsApp" data-titulo="{{ $comunicado->titulo }}" data-contenido="{{ $comunicado->contenido }}">
                                                ...
                                                @if (!$comunicado->fecha_envio)
                                                    ...
                                                    <form action="{{ route('comunicados.enviar', $comunicado->id) }}" method="POST" class="inline-block">
                                                        @csrf
                                                ...
                                                <form action="{{ route('comunicados.destroy', $comunicado->id) }}" method="POST" class="inline-block">
                                                    @csrf @method('DELETE')
                    ...
                    <div class="space-y-4 md:hidden">
                        @forelse ($comunicados as $comunicado)
                            <div class="bg-white border rounded-lg shadow-sm p-4">
                                ...
                                <p class="text-sm mt-1 text-gray-600"><span class="font-medium text-gray-800">Por:</span> {{ $comunicado->user->name }}</p>
