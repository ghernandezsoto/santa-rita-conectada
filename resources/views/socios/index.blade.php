    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('socios.create') }}"
               class="px-4 py-2 bg-amber-500 text-white rounded-lg hover:bg-amber-600 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2">
                Agregar Nuevo Socio
            </a>
        </div>
    </x-slot>
                    ...
                    <form action="{{ route('socios.index') }}" method="GET">
                        <div class="flex items-center">
                            <input type="text" name="search" placeholder="Buscar por nombre o RUT..."
                                   class="border-gray-300 focus:border-emerald-600 focus:ring-emerald-600 rounded-md shadow-sm w-full sm:w-1/3"
                                   value="{{ request('search') }}">
                            <button type="submit"
                                    class="ml-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:ring-offset-2">
                                Buscar
                            </button>
                        </div>
                    </form>
                                ...
                                <tr class="border-b hover:bg-slate-50">
                                    ...
                                    <td class="py-3 px-4">
                                        <a href="{{ route('socios.show', $socio->id) }}"
                                           class="block truncate font-medium text-emerald-800 hover:underline"
                                           title="{{ $socio->nombre }}">
                                            {{ $socio->nombre }}
                                        </a>
                                    </td>
                    ...
                    @forelse ($socios as $socio)
                        <div class="bg-white p-4 rounded-lg shadow border border-gray-200">
                            <div class="flex justify-between items-center mb-2">
                                <a href="{{ route('socios.show', $socio->id) }}" class="font-bold text-emerald-800">{{ $socio->nombre }}</a>
                                <span class="py-1 px-3 rounded-full text-xs font-semibold {{ $socio->estado === 'Activo' ? 'bg-emerald-100 text-emerald-800' : 'bg-red-200 text-red-700' }}">
                                    {{ $socio->estado }}
                                </span>
                            </div>
                            ...
                            <div class="mt-4 flex items-center gap-4">
                                <a href="{{ route('socios.edit', $socio->id) }}" class="text-amber-600 hover:text-amber-700 font-medium text-sm">Editar</a>
                                <form action="{{ route('socios.destroy', $socio->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-700 font-medium text-sm" onclick="return confirm('¿Estás seguro?')">
                                        Eliminar
                                    </button>
                                </form>
                            </div>
