        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-6">
                <div class="bg-white p-5 sm:p-6 rounded-lg shadow-md">
                    ...
                    <p class="text-2xl sm:text-3xl font-bold mt-2 text-emerald-600 tabular-nums">
            ...
            <div class="mb-6 flex flex-col sm:flex-row sm:flex-wrap gap-3 sm:gap-4">
                <a href="{{ route('transacciones.create', ['tipo' => 'Ingreso']) }}"
                   class="px-5 py-2.5 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 focus:ring-4 focus:outline-none focus:ring-emerald-300 rounded-lg text-center"
                   role="button" aria-label="Registrar ingreso">
                    Registrar Ingreso
                </a>

                <a href="{{ route('transacciones.create', ['tipo' => 'Egreso']) }}"
                   class="px-5 py-2.5 text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 rounded-lg text-center"
                   role="button" aria-label="Registrar egreso">
                    Registrar Egreso
                </a>

                <a href="{{ route('transacciones.exportar') }}"
                   class="inline-flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-medium text-gray-700 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-center"
                   role="button" aria-label="Exportar transacciones a Excel">
                                ...
                                <tbody class="text-gray-700">
                                    @forelse ($transacciones as $transaccion)
                                        <tr class="border-b hover:bg-slate-50">
                                            ...
                                            {{-- Descripción: truncada dentro de celda de ancho fijo --}}
                                            <td class="py-3 px-4 max-w-0">
                                                <div class="truncate" title="{{ $transaccion->descripcion }}">
                                            ...
                                            {{-- Socio: truncado --}}
                                            <td class="py-3 px-4 max-w-0">
                                                <div class="truncate text-sm text-gray-600" title="{{ $transaccion->socio->nombre ?? '' }}">
                                                    {{ $transaccion->socio->nombre ?? 'N/A' }}
                                                </div>
                                            </td>

                                            {{-- Monto: alineado a la derecha, sin salto --}}
                                            <td class="py-3 px-4 font-medium whitespace-nowrap text-right tabular-nums {{ $transaccion->tipo == 'Ingreso' ? 'text-emerald-600' : 'text-red-600' }}">
                            ...
                            <article class="bg-white border rounded-lg p-4 shadow-sm">
                                <div class="flex items-start justify-between gap-3">
                                    <div class="min-w-0">
                                        ...
                                        <p class="mt-2 text-sm text-gray-700 break-words" title="{{ $transaccion->descripcion }}">
                                            {{ $transaccion->descripcion }}
                                        </p>
                                        <p class="mt-2 text-xs text-gray-500">
                                            Aporte de:
                                            <span class="font-medium">{{ $transaccion->socio->nombre ?? 'N/A' }}</span>
                                        </p>
                                    ...
                                    <div class="text-right ml-2 flex-shrink-0">
                                        <p class="font-medium tabular-nums {{ $transaccion->tipo == 'Ingreso' ? 'text-emerald-600' : 'text-red-600' }}">
                                            {{ $transaccion->tipo == 'Ingreso' ? '+' : '-' }}
                                            ${{ number_format($transaccion->monto, 0, ',', '.') }}
                                        </p>
                                        <div class="mt-2 flex items-center justify-end gap-3">
                                            ...
                                            <form action="{{ route('transacciones.destroy', $transaccion->id) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:text-red-700 text-sm" onclick="return confirm('¿Estás seguro de que quieres eliminar esta transacción?')">
                                                    Eliminar
                                                </button>
                                            </form>
                                        </div>
